...
function RevistasPage() {
  ...
  useEffect(() => {
    const fetchRevistas = async () => {
      try {
        ...
        const [revistasResponse] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/revistas/`)
        ]);

        const revistasData = revistasResponse.data;
        console.log(revistasData);

        const uniqueInstitutions = Array.from(
          new Set(revistasData.map((revista) => revista.publisher).filter(Boolean))
        );

        setInstitutions(uniqueInstitutions);
        setRevistas(revistasData);
        setFilteredRevistas(revistasData); // Asegúrate de inicializar los datos filtrados también
  ...
  const handleSearch = (query) => {
    ...
    const searched = revistas.filter((revista) => {
      const normalizedName = normalizeString(revista.repository_name || "");
            ...
            <Grid item xs={12} sm={6} md={4} key={revista.id}>
              ...
              >
                ...
                <Box
                  ...
                  alt={`Portada de ${revista.repository_name}`}
                  sx={{
                    height: 500,
                    width: "100%",
                    objectFit: "cover",
